...
pair<RandIt, RandIt> FindStartsWith(RandIt range_begin, RandIt range_end, char prefix) {
    const string prefix_str(1, prefix);
    pair<RandIt, RandIt> result = equal_range(range_begin, range_end, prefix_str,
    [&prefix_str](const string& l_word, const string& r_word) {
        return l_word.compare(0, prefix_str.length(), r_word.substr(0, prefix_str.length())) < 0;
    });
